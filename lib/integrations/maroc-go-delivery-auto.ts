...
export async function autoCreateMarocGoDeliveryParcelForOrder(params: {
  ...
  if (!cityKey) {
    const cityMatch = await normalizeOrderCityById(order.id, admin, 'maroc-go-delivery')
    cityKey = Number(cityMatch.cityKey || 0) || 0
  }

  if (!cityKey) {
    return {
      warning: `Ville non reconnue pour Maroc Go Delivery: ${String(order.city || '').trim()}`,
      trackingNumber: '',
    }
  }
  ...
  if (!resolvedShopKey) {
    return {
      warning: 'Aucun shop Maroc Go Delivery configuré pour ce store.',
      trackingNumber: '',
    }
  }
