...
export async function POST(request: NextRequest) {
  try {
    ...
    const buildOrdersQuery = (withFilter: boolean) => {
      let query = admin
        .from('orders')
        .select('id, tracking_number, status, exchange_status, exchange_original_order_id')
        .eq('store_id', storeId)
    ...
    if (ordersResult.error) {
      console.warn('Rapid Delivery sync-all: filtre SQL indisponible, repli JS', ordersResult.error.message)
