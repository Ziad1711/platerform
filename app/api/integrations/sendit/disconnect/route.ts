...
export async function POST(request: Request) {
  try {
    ...
    // Delete sendit_configs
    await admin.from('sendit_configs').delete().eq('store_id', storeId)

    // Delete store_integrations
    await admin.from('store_integrations').delete().eq('store_id', storeId).eq('provider_slug', 'sendit')
    ...
    const message = error instanceof Error ? error.message : 'SENDIT_DISCONNECT_FAILED'
