...
export async function POST(request: Request) {
  try {
    ...
    const { error: storeIntegrationError } = await admin
      ...
      .delete()
      .eq('store_id', storeId)
      .eq('provider_slug', 'maroc-go-delivery')

    if (storeIntegrationError && !isMissingDbObjectError(storeIntegrationError)) throw storeIntegrationError

    const { error: deleteConfigError } = await admin.from('maroc_go_delivery_configs').delete().eq('store_id', storeId)
    ...
    if (integrationId) {
      ...
      if (!remainingConfigs || remainingConfigs.length === 0) {
        const purgeTargets = ['delivery_shops', 'delivery_entity_mappings']
    ...
    const message = error instanceof Error ? error.message : 'MAROC_GO_DELIVERY_DISCONNECT_FAILED'
