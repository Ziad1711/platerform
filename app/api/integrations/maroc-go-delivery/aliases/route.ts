...
export async function GET(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'MAROC_GO_DELIVERY_ALIASES_GET_FAILED'
  ...
  try {
    ...
    if (!resolvedCityKey) {
      ...
      if (provider?.id) {
        ...
        resolvedCityKey = Number(rateRow?.external_city_key || 0) || 0
      }
    }
    ...
    const message = error instanceof Error ? error.message : 'MAROC_GO_DELIVERY_ALIASES_POST_FAILED'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    const admin = createAdminClient()
    const { error } = await admin.from('maroc_go_delivery_city_aliases').delete().eq('alias', alias)

    if (error) throw error
    return NextResponse.json({ ok: true })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'MAROC_GO_DELIVERY_ALIASES_DELETE_FAILED'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
