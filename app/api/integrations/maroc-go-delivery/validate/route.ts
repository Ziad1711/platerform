...
import { listMarocGoDeliveryShops } from '@/lib/integrations/maroc-go-delivery'
import { listUserStores } from '@/lib/integrations/maroc-go-delivery-connect'

function toMarocGoDeliveryErrorMessage(error: unknown) {
  console.error('Maroc Go Delivery validate error:', error)

  const fallbackMessage = 'MAROC_GO_DELIVERY_VALIDATE_FAILED'
