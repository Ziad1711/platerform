...
import { downloadAmeexDeliveryNoteHtml } from '@/lib/integrations/ameex'
...
export async function GET(_request: Request, context: { params: Promise<{ key: string }> }) {
  try {
    ...
    const raw = await downloadAmeexDeliveryNoteHtml(credentials.apiId, credentials.apiKey, voucherKey)
    ...
    return new NextResponse(html, {
      ...
      headers: {
        ...
        'Content-Disposition': `inline; filename="ameex-bon-${voucherKey}.html"`,
      },
    })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'AMEEX_VOUCHER_PDF_FAILED'
