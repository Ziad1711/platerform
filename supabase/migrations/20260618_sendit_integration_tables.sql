create table if not exists public.sendit_configs (
  ...
  default_pickup_district_id text null,
  default_packaging_id text null,
  ...
  default_print_format integer not null default 1,
...
create policy sendit_configs_select on public.sendit_configs
  for select using (store_id in (select store_id from public.store_members where user_id = auth.uid()));
...
create policy sendit_configs_manage on public.sendit_configs
  for all using (store_id in (select store_id from public.store_members where user_id = auth.uid()))
  with check (store_id in (select store_id from public.store_members where user_id = auth.uid()));

insert into public.integration_provider_metrics (provider_id, connected_users_count, updated_at)
values ('5998e563-96ed-47cc-881a-43f41827f858', 0, now())
on conflict (provider_id) do nothing;

update public.integration_providers
set name = 'Sendit', slug = 'sendit', category = coalesce(category, 'delivery'), is_active = true
where id = '5998e563-96ed-47cc-881a-43f41827f858';