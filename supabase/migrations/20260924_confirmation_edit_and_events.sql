-- ============================================================
-- Module Confirmation — édition de la commande avant confirmation
-- 1) Types d'événements supplémentaires (client / produits / livraison)
-- 2) RLS orders : les rôles en lecture seule ne peuvent plus écrire
-- 3) RPC transactionnelle de modification (produits, prix, client, livraison)
-- ============================================================

-- ---------- 1. Types d'événements supplémentaires ----------
  ...
  add constraint order_confirmation_events_type_check check (
    event_type = any (
      array[
        ...
        'order_items_updated'::text,
        'delivery_information_updated'::text,
...
-- ---------- 2. RLS orders : écriture réservée aux rôles opérationnels ----------
-- Avant : tout membre du store (y compris « viewer ») pouvait modifier une
-- commande. Le rôle de lecture seule ne doit plus pouvoir écrire.
...
using (
  exists (
    ...
    where sm.store_id = orders.store_id
      ...
      and sm.role in ('owner', 'admin', 'confirmation', 'delivery', 'accountant', 'stock_manager')
  )
  or exists (
...
-- ---------- 3. RPC transactionnelle de modification ----------
-- Une seule transaction : client, livraison, produits, prix, journal.
-- Le prix d'achat n'est jamais fourni par le client : il est toujours résolu
-- côté serveur (coût de la variante, sinon coût par défaut du produit).
...
begin
  ...
  if not exists (
    ...
    where sm.store_id = v_order.store_id
      ...
      and sm.role in ('owner', 'admin', 'confirmation')
  ) and not exists (
    select 1
  ...
  if p_items is not null then
    ...
    for v_item in select * from jsonb_array_elements(p_items) loop
      ...
      -- Le produit doit appartenir au store de la commande.
      ...
      -- Le prix d'achat est toujours résolu ici, jamais fourni par le client.
  ...
  set customer_name = v_customer_name,
      ...
      address = v_address,
      city = v_city,
      delivery_city_external_id = coalesce(p_city_key::text, delivery_city_external_id),
...
comment on function public.rpc_update_confirmation_order_details(uuid, text, text, text, text, numeric, text, uuid, text, jsonb) is
  'Modification transactionnelle d''une commande encore dans le circuit de confirmation (client, livraison, produits, prix). Le prix d''achat est toujours résolu côté serveur.';
