-- ============================================================
-- Module Confirmation — action atomique de confirmation
-- (partie 4/4 de la migration 20260924_confirmation_*)
-- ============================================================

-- ---------- 6. Action de confirmation atomique ----------
-- Toute action de confirmation passe par cette fonction : c'est le serveur qui
-- décide du numéro d'appel, de la limite du store et de l'annulation automatique.
...
comment on function public.rpc_order_confirmation_action(uuid, text, timestamptz, text, text, text, integer) is
  'Action atomique de confirmation (NO_ANSWER, POSTPONE, CANCEL, CONFIRM). Le serveur calcule le numéro d''appel, applique la limite du store et journalise l''événement.';

revoke all on function public.rpc_order_confirmation_action(uuid, text, timestamptz, text, text, text, integer) from public;
grant execute on function public.rpc_order_confirmation_action(uuid, text, timestamptz, text, text, text, integer) to authenticated, service_role;
