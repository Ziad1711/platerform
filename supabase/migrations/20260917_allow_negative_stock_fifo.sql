...
-- Une sortie de commande ne doit plus être bloquée lorsqu'aucun lot FIFO
-- n'est disponible : la quantité manquante sort sans lot d'entrée, ce qui
-- fait passer le stock du produit en négatif au lieu d'annuler le statut.
-- ============================================================

alter table public.inventory_consumptions
  alter column inventory_in_id drop not null;
...
begin
  ...
  loop
    ...
    while v_needed > 0 loop
      ...
      if not found then
        -- Stock négatif autorisé : la quantité manquante sort sans lot FIFO.
        -- Le coût retenu est le dernier coût d'achat connu, sinon le coût
        ...
        select coalesce(
                 (select im.unit_cost
                   ...
                   limit 1),
                 (select p.default_purchase_cost from public.products p where p.id = v_item.product_id),
                 0
               )
        into v_shortage_cost;
