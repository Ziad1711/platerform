-- FIFO tenant compte de la variante (mode `variant`) + confirmation de la configuration
-- du stock/variantes (packs quantité) pour ne plus redemander à chaque synchronisation.

alter table public.products
  add column if not exists stock_setup_confirmed_at timestamp with time zone null;

comment on column public.products.stock_setup_confirmed_at is
  'Horodatage de confirmation de la configuration stock/variantes (packs quantité). NULL = configuration encore en attente.';
...
begin
  ...
  for v_item in
    ...
    order by oi.id
  loop
    -- Multiplicateur de pack + mode de gestion du stock du produit.
    select
    ...
    while v_needed > 0 loop
      ...
      where im.store_id = v_order.store_id
        ...
        -- Mode "stock par variante": on n'utilise que le stock de la variante
        -- concernée (avec repli sur le stock produit sans variante si nécessaire).
        and (
